...
class FarmerService:
    ...
    def create_profile(cls, *, user: User, aadhar_no: str, land_area: float, soil_type: str, irrigation: str) -> FarmerProfile:
        ...
        return FarmerProfile.objects.create(
            user=user, aadhar_no=aadhar_no, land_area=land_area,
    ...
    def request_manual_soil_report(cls, *, user: User, land_area: float | None = None, previous_crop: str | None = None) -> ManualSoilReport:
        ...
        report, _ = ManualSoilReport.objects.select_for_update().get_or_create(
            farmer=user,
            defaults={"land_area": land_area, "previous_crop": previous_crop},
        )
    ...
    @classmethod
    @transaction.atomic
    def create_produce_listing(cls, *, user: User, category: str, title: str, price: Decimal | float | str,
        ...
        return MarketplaceListing.objects.create(
            listed_by=user, wing="PRODUCE", category=category, title=title.strip(),
            variety_or_brand=variety_or_brand.strip() if variety_or_brand else None,
            price=Decimal(str(price)), unit_of_measure=unit_of_measure.strip(),
            available_stock=available_stock, min_order_quantity=min_order_quantity,
            description=description.strip(), harvest_date=harvest_date,
            is_organic=is_organic, grade=grade, specifications=dict(specifications or {}),
    ...
    def update_listing(cls, *, user: User, listing_id: int, changes: Mapping[str, Any]) -> MarketplaceListing:
        ...
        allowed = {"category", "title", "variety_or_brand", "price", "unit_of_measure", "available_stock",
                   "min_order_quantity", "description", "harvest_date", "is_organic", "grade", "specifications", "status"}
        unknown = set(changes) - allowed
        if unknown:
    ...
    @classmethod
    @transaction.atomic
    def respond_to_trade_proposal(cls, *, user: User, proposal_id: int, action: str) -> DirectTradeProposal:
        cls._ensure_farmer(user)
    ...
    def place_input_order(cls, *, user: User, listing_id: int, quantity: float, payment_method: str, delivery_address: str) -> InputOrder:
        ...
        order = InputOrder.objects.create(
            farmer=user, product=product, quantity=quantity, total_amount=total,
            payment_method=payment_method, delivery_address=delivery_address.strip(), status="PENDING",
        )
        EscrowTransaction.objects.create(
            item_purchased=product, vendor=product.listed_by, purchaser=user,
            amount_paid=total, payment_status="COMPLETED", security_token=f"ORDER-{order.order_id}",
        )
        return order
