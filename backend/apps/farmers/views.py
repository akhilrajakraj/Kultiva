...
def edit_listing(request):
    ...
    if not listing_id: messages.error(request, "Listing ID is required."); return redirect("farmer_manage_crops")
    listing = get_object_or_404(MarketplaceListing, pk=listing_id, listed_by=user, wing="PRODUCE")
    if request.method == "POST":
        try:
            changes = {f: request.POST.get(f) for f in ("category", "title", "variety_or_brand", "price", "unit_of_measure", "available_stock", "min_order_quantity", "description", "harvest_date", "grade") if f in request.POST}
            if "stock" in request.POST and "available_stock" not in changes: changes["available_stock"] = request.POST.get("stock")
            if "price" in changes: changes["price"] = Decimal(changes["price"])
            for f in ("available_stock", "min_order_quantity"):
                if f in changes: changes[f] = float(changes[f])
            if "is_organic" in request.POST: changes["is_organic"] = request.POST.get("is_organic") == "on"
            FarmerService.update_listing(user=user, listing_id=listing.id, changes=changes); messages.success(request, "Listing updated successfully."); return redirect("farmer_manage_crops")
        except (ValueError, TypeError) as exc: messages.error(request, str(exc))
...
def delete_listing(request, listing_id: int):
    user = _farmer(request)
    if user is None: return redirect("index")
    try: FarmerService.delete_listing(user=user, listing_id=listing_id); messages.success(request, "Listing deleted successfully.")
...
def submit_soil_report(request):
    ...
    if request.method == "POST":
        try:
            ...
            if address_id is not None and not user.addresses.filter(pk=address_id).exists(): raise ValueError("The selected farm address does not belong to this farmer.")
...
def proposals(request):
    user = _farmer(request)
    if user is None: return redirect("index")
    return render(request, "domains/farmer/proposals.html", selectors.list_proposals(user=user))

@login_required
def proposal_detail(request, proposal_id: int):
    user = _farmer(request)
    if user is None: return redirect("index")
    proposal = selectors.get_proposal(user=user, proposal_id=proposal_id)
...
def send_proposal(request):
    user = _farmer(request)
    if user is None: return redirect("index")
    if request.method != "POST": return redirect("farmer_home")
    try:
        proposal = FarmerService.send_trade_proposal(user=user, listing_id=int(request.POST["listing_id"]), buyer_id=int(request.POST["buyer_id"]), message=request.POST.get("message", ""), hide_listing=request.POST.get("hide_listing") == "on" or request.POST.get("visibility_action") == "HIDE")
        messages.success(request, "Trade proposal sent successfully."); return redirect("farmer_proposal_detail", proposal_id=proposal.id)
    except (ValueError, TypeError) as exc: messages.error(request, str(exc)); return redirect("farmer_home")
...
def respond_proposal(request, proposal_id: int):
    ...
    if user is None: return redirect("index")
    if request.method == "POST":
        try: FarmerService.respond_to_trade_proposal(user=user, proposal_id=proposal_id, action=request.POST.get("action", ""), farmer_message=request.POST.get("farmer_message", "")); messages.success(request, "Proposal response recorded.")
        except (ValueError, TypeError) as exc: messages.error(request, str(exc))
...
def generate_trade_qr(request, proposal_id: int):
    ...
    if user is None: return redirect("index")
    if request.method == "POST":
        try: FarmerService.generate_trade_token(user=user, proposal_id=proposal_id); messages.success(request, "Secure trade token generated successfully.")
        except (ValueError, TypeError) as exc: messages.error(request, str(exc))
    return redirect("farmer_proposal_detail", proposal_id=proposal_id)
...
def input_market(request):
    user = _farmer(request)
    if user is None: return redirect("index")
    return render(request, "farmer_input_market.html", {"products": selectors.list_input_products()})

@login_required
def input_detail(request, listing_id: int):
    user = _farmer(request)
    if user is None: return redirect("index")
    product = selectors.get_input_product(listing_id=listing_id)
    return render(request, "farmer_input_detail.html", {"product": product, "formatted_specs": {k.replace("_", " "): v for k, v in (product.specifications or {}).items()}, "has_purchased": selectors.has_purchased_input(user=user, product=product)})

@login_required
def checkout(request, listing_id: int):
    user = _farmer(request)
    if user is None: return redirect("index")
    product = selectors.get_input_product(listing_id=listing_id); address = selectors.get_primary_address(user=user)
    qty = float(request.POST.get("quantity", 1)) if request.method == "POST" else 1
    return render(request, "farmer_checkout.html", {"product": product, "address": address, "quantity": qty, "total": product.price * Decimal(str(qty)) + Decimal("20.00")})

@login_required
def process_order(request, listing_id: int):
    user = _farmer(request)
    if user is None: return redirect("index")
    if request.method != "POST": return redirect("farmer_checkout", listing_id=listing_id)
    try:
        qty = float(request.POST.get("quantity", 1)); address = selectors.get_primary_address(user=user)
        address_text = f"{address.village}, {address.district}, {address.state} - {address.pincode}" if address else "Address Pending"
        order = FarmerService.place_input_order(user=user, listing_id=listing_id, quantity=qty, payment_method=request.POST.get("payment_mode", "UPI"), delivery_address=address_text)
        messages.success(request, f"Order {order.order_id} placed successfully."); return redirect("farmer_order_details", order_id=order.order_id)
    except (ValueError, TypeError) as exc: messages.error(request, str(exc)); return redirect("farmer_checkout", listing_id=listing_id)

@login_required
def payment_gateway(request, listing_id: int):
    user = _farmer(request)
    if user is None: return redirect("index")
    if request.method == "POST": return process_order(request, listing_id)
    product = selectors.get_input_product(listing_id=listing_id); qty = float(request.GET.get("quantity", 1))
...
def order_detail(request, order_id: str):
    ...
    return render(request, "farmer_order_details.html", {"order": order, "product": order.product, "txn": selectors.get_order_transaction(order_id=order.order_id)})
